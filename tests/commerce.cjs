...
(async()=>{
 ...
 const optedOut=await quoteRoute.POST(req({...payload,customer:{...payload.customer,sendCopyToCustomer:false}}));assert.equal(optedOut.status,200);const optOutCall=calls.filter(c=>c.name==='capture-pavilion-lead').at(-1);assert.equal(optOutCall.body.config.emailCopyRequested,false);assert.equal(optOutCall.body.customer.sendCopyToCustomer,false);
