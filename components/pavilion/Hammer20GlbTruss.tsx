...
import glbAsset from "@/assets/hammer20_truss.glb.asset.json";
...
export type Hammer20GlbAdjust = {
  scale: { x: number; y: number; z: number };
  offset: { x: number; y: number; z: number };
  rotation: { x: number; y: number; z: number };
};

export const HAMMER20_GLB_DEFAULT_ADJUST: Hammer20GlbAdjust = {
  scale: { x: 1, y: 1, z: 1 },
  offset: { x: 0, y: 0, z: 0 },
  rotation: { x: 0, y: 0, z: 0 },
};

/** Drop-in 20′ hammer-beam truss rendered from a single GLB. Mirrors the
...
export function Hammer20GlbTruss({
  span,
  z,
  baseY,
  color,
