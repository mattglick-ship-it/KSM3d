...
export function QuoteCheckout({open,onOpenChange,design,intent,capture}:{open:boolean;onOpenChange:(v:boolean)=>void;design:Design;intent:'quote'|'order';capture:()=>string|undefined}){
 ...
 const [customer,setCustomer]=useState<QuoteCustomer>({name:'',email:'',phone:'',fulfillment:'pickup',address:'',notes:''});
 ...
 const shippingRequest=useRef(0),savedFingerprint=useRef(''),attemptFingerprint=useRef(''),submission=useRef('');
 const summary=useMemo(()=>pavilionSummary(design,projectName,pricing),[design,projectName,pricing]);
 const update=(key:keyof QuoteCustomer,value:string)=>{setCustomer(c=>({...c,[key]:value}));setSaved(false);setError('');if(key==='address'||key==='fulfillment'){shippingRequest.current++;setShipping(null);setShippingBusy(false);setShippingError('')}};
 ...
 <fieldset><legend>Delivery or pickup</legend><RadioGroup value={customer.fulfillment} onValueChange={v=>update('fulfillment',v)} className="quote-fulfillment"><label><RadioGroupItem value="pickup"/>Pickup at KSM</label><label><RadioGroupItem value="delivery"/>Delivery</label></RadioGroup></fieldset>
 ...
 <p className="quote-note">Your quote is saved with KSM and an email copy is requested. Estimated lead time: 4–6 weeks from order confirmation.</p>
