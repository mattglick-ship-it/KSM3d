import {z} from 'zod';
import {DEFAULT_CONFIG,PAVILION_SIZES,ROOF_MATERIALS,WOOD_FINISHES,type PavilionConfig} from '@/lib/pavilion-config';
import type {ViewPreset} from './Scene';
export const designSchema=z.object({
 version:z.literal(1),
 config:z.object({width:z.number(),length:z.number(),height:z.union([z.literal(8),z.literal(9),z.literal(10)]),roof:z.literal('gable'),post:z.enum(['square','brace']),truss:z.enum(['king','arch','hammer']),woodId:z.string().refine(v=>WOOD_FINISHES.some(x=>x.id===v)),roofId:z.string().refine(v=>ROOF_MATERIALS.some(x=>x.id===v)),trussPlates:z.boolean(),braceScale:z.number().min(0.25).max(1.5),snowGuards:z.boolean(),snowRail:z.boolean().optional(),rafterTail:z.enum(['standard','scroll']),gableFascia:z.boolean(),gableOverhang:z.boolean(),hideBackTruss:z.boolean().optional()}).refine(c=>PAVILION_SIZES.some(s=>s.width===c.width&&s.length===c.length&&s.width<24),'Choose an available pavilion size'),
 tableStain:z.string().regex(/^#[0-9a-f]{6}$/i).nullable(),deckStain:z.string().regex(/^#[0-9a-f]{6}$/i).nullable(),
 notes:z.string().max(2000),
 furniture:z.enum(['none','picnic','sectional','dining','patio','grill']),
});
export type Design=z.infer<typeof designSchema>;
export const initialDesign:Design={version:1,config:{...DEFAULT_CONFIG,height:8,roof:'gable',truss:'king',roofId:'ss-slate-gray'},tableStain:null,deckStain:null,notes:'',furniture:'picnic'};
export type SceneInputs={config:PavilionConfig;view:ViewPreset;showRoof:boolean;showPad:boolean;showBackyard:boolean;showTable:boolean;showSectional:boolean;showEgg:boolean;showTv:boolean;showDining:boolean;showPatio:boolean;tableStain:string|null;deckStain:string|null;measureEnabled:boolean;showDimensions:boolean};
