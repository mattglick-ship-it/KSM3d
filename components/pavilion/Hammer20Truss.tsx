...
export type Hammer20Adjust = {
  scale: { x: number; y: number; z: number };
  offset: { x: number; y: number; z: number };
  rotation: { x: number; y: number; z: number };
};

export const HAMMER20_GLB_DEFAULT_ADJUST: Hammer20Adjust = {
  scale: { x: 1, y: 1, z: 1 },
  offset: { x: 0, y: 0, z: 0 },
  rotation: { x: 0, y: 0, z: 0 },
};

/** Drop-in 20′ hammer-beam truss rebuilt from dimensional measurements using procedural solids. Mirrors the
...
export function Hammer20Truss({
  span,
  z,
  baseY,
  color,
  adjust = HAMMER20_GLB_DEFAULT_ADJUST,
}: {
  span: number;
  z: number;
  baseY: number;
  color: string;
  adjust?: Hammer20Adjust;
}) {
  const grainTex = useWoodTexture(Math.PI / 2, 1, 1, "hammer20glb");
  ...
  const cloned = useMemo(() => createHammerScene(20, sharedMat), [sharedMat]);
