...
export type Hammer16Adjust = {
  scale: { x: number; y: number; z: number };
  offset: { x: number; y: number; z: number };
  rotation: { x: number; y: number; z: number };
};

export const HAMMER16_GLB_DEFAULT_ADJUST: Hammer16Adjust = {
  scale: { x: 1, y: 1, z: 1 },
  offset: { x: 0, y: 0, z: 0 },
  rotation: { x: 0, y: 0, z: 0 },
};

/** Drop-in 16′ hammer-beam truss rebuilt from dimensional measurements using procedural solids. Mirrors the
...
export function Hammer16Truss({
  ...
  const cloned = useMemo(() => createHammerScene(16, sharedMat), [sharedMat]);
